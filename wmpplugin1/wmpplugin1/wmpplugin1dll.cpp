// wmpplugin1dll.cpp : Implementation of DLL Exports.
//
...
OBJECT_ENTRY(CLSID_Wmpplugin1, CWmpplugin1)
OBJECT_ENTRY(CLSID_Wmpplugin1PropPage, CWmpplugin1PropPage)
END_OBJECT_MAP()
...
STDAPI DllRegisterServer(void)
{
    ...
    DMO_PARTIAL_MEDIATYPE* mt = new DMO_PARTIAL_MEDIATYPE[CWmpplugin1::k_dwValidSubtypesCount];
    if (NULL == mt)
    {
        return E_OUTOFMEMORY;
    }

    ::ZeroMemory( mt, ( sizeof( DMO_PARTIAL_MEDIATYPE ) * CWmpplugin1::k_dwValidSubtypesCount));

    for( DWORD i = 0; i < CWmpplugin1::k_dwValidSubtypesCount; i++)
    {
        mt[i].type = MEDIATYPE_Video;
        mt[i].subtype = *CWmpplugin1::k_guidValidSubtypes[i];
    }
    ...
    hr = spRegistrar->WMPRegisterPlayerPlugin(
                    ...
                    CWmpplugin1::k_dwValidSubtypesCount, // No. media types supported by plug-in
...

{
    ...
    hr = spRegistrar->WMPUnRegisterPlayerPlugin(WMP_PLUGINTYPE_DSP, CLSID_Wmpplugin1);
 
    if(TRUE == IsVistaOrLater())
    {
        // Also unregister from the MF pipeline
        hr = spRegistrar->WMPUnRegisterPlayerPlugin(WMP_PLUGINTYPE_DSP_OUTOFPROC, CLSID_Wmpplugin1);
    }
